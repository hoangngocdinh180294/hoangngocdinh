...
@include('themes.mspace.home.sections.menu')
            <div class="section-title section-bg">
              <div class="section__inner">
                <h1 class="ui-title-page">Tất cả tin tức Greenhouse</h1>
              </div>
            </div>
          </div>
                ...
                <div class="col-md-8">
                  ...
                  <article class="post post-5 clearfix">
                    ...
                    <div class="entry-main">
                      ...
                      <h2 class="entry-title"><a href="{{route('posts.detail', ['slug' => $item->slug])}}">{!!App\Helpers\Theme::truncateWords(strip_tags($item->title), 30)!!}</a></h2>
                      </div>
                      <div class="entry-meta"><span class="entry-meta__item">By<a href="{{route('posts.detail', ['slug' => $item->slug])}}" class="entry-meta__slug"> greenhouse</a></span><span class="entry-meta__item"><a href="{{route('posts.detail', ['slug' => $item->slug])}}" class="entry-meta__slug"></a></span><span class="entry-meta__item"><i class="icon pe-7s-comment"></i><a href="{{route('posts.detail', ['slug' => $item->slug])}}" class="entry-meta__slug"></a></span></div>
                      <div class="entry-content">
                        <p>{!!App\Helpers\Theme::truncateWords(strip_tags($item->body), 98)!!}</p>
                      </div>
                      <div class="entry-footer"><a href="{{route('posts.detail', ['slug' => $item->slug])}}" class="btn-slug">Đọc tiếp</a></div>
                    </div>
                  </article>
                  @endforeach
                      ...
                      <div class="widget-content">
                        ...
                        <article class="post post-3 post-3_mod-a clearfix">
                          ...
                          <div class="entry-main">
                            ...
                            <h2 class="entry-title"><a href="{{route('posts.detail', ['slug' => $item1->slug])}}">{{$item1->title}}</a></h2>
                            </div>
                            @if ($item1->project_category_id == 1)
                            <div class="entry-meta"><span class="category color-4">đọc tiếp</span><span class="entry-meta__item"><i class="icon pe-7s-comment"></i><a href="{{route('posts.detail', ['slug' => $item1->slug])}}" class="entry-meta__slug">greenhouse</a></span></div>
                            @endif
                            @if ($item1->project_category_id == 2)
                            <div class="entry-meta"><span class="category color-7">đọc tiếp</span><span class="entry-meta__item"><i class="icon pe-7s-comment"></i><a href="{{route('posts.detail', ['slug' => $item1->slug])}}" class="entry-meta__slug">greenhouse</a></span></div>
                            @endif
                            @if ($item1->project_category_id == 3)
                            <div class="entry-meta"><span class="category color-4">đọc tiếp</span><span class="entry-meta__item"><i class="icon pe-7s-comment"></i><a href="{{route('posts.detail', ['slug' => $item1->slug])}}" class="entry-meta__slug">greenhouse</a></span></div>
                            @endif
                            @if ($item1->project_category_id == 4)
                            <div class="entry-meta"><span class="category color-12">đọc tiếp</span><span class="entry-meta__item"><i class="icon pe-7s-comment"></i><a href="{{route('posts.detail', ['slug' => $item1->slug])}}" class="entry-meta__slug">greenhouse</a></span></div>
                            @endif
                          </div>
                        </article>
                        @endforeach
